...
def _categorize_message_type(message: Dict[str, Any]) -> str:
    ...
    if any(token in type_field for token in ("voice", "audio", "语音", "录音")) or "voice" in local_type:
        return "voice"
    if any(token in type_field for token in ("video", "视频", "短视频")):
        return "video"
...
def _categorize_session(meta: Dict[str, object]) -> str:
    ...
    if "chatroom" in session_id or "群" in session_type or "group" in session_type:
        return "group"
    if "single" in session_type or "私" in session_type:
        return "single"
