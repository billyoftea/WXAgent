...
import 'export_state_service.dart';
...
class ChatExportService {
      ...
      final messageItems = messages
          .map(
            (msg) {
              ...
              return _buildPublicMessagePayload(msg, senderName);
            },
          )
          .toList();
  ...
  /// [session] 聊天会话
  /// [messages] 所有消息（全量）
  /// [filePath] 导出文件路径（如果为 null，则使用默认路径）
  /// [onlyNewMessages] 如果为 true，则只追加新消息；如果为 false，则覆盖整个文件
  /// 
  /// 返回导出状态，包含导出进度和历史信息
  Future<ExportState?> exportToJsonIncremental(
    ...
    try {
      ...
      if (filePath == null) {
        ...
        filePath = '${appDir.path}${Platform.pathSeparator}EchoTrace_Export'
            '${Platform.pathSeparator}${sanitizedName}.json';
      }
      ...
      // 获取现有的导出状态
      ExportState? existingState;
      if (onlyNewMessages && await file.exists()) {
        existingState = await ExportStateService.getExportState(filePath);
      }
      ...
      // 确定要导出的消息范围
      List<Message> messagesToExport = messages;
      List<Map<String, dynamic>> existingMessages = [];

      if (onlyNewMessages && existingState != null) {
        // 只导出新增的消息（localId > lastExportedLocalId）
        messagesToExport = messages
            .where((msg) => msg.localId > existingState!.lastExportedLocalId)
            .toList();
        ...
        if (await file.exists()) {
          try {
            ...
            existingMessages = existingMessages
                .map(_normalizeLegacyMessagePayload)
                .toList();
          } catch (e) {
            await logger.warning(
              'ChatExportService',
              '读取现有 JSON 文件失败，将覆盖: $e',
            );
            existingMessages = [];
          }
        }
      ...
      // 转换消息为 JSON 格式
      final newMessageItems = messagesToExport
          .map(
            (msg) {
              final isSend = msg.isSend == 1;
              final senderName = _resolveSenderDisplayName(
      ...
      final data = {
        ...
        'exportMetadata': {
          ...
          'firstExportTime': existingState?.firstExportTime
              .toIso8601String() ??
              DateTime.now().toIso8601String(),
          'lastExportTime': DateTime.now().toIso8601String(),
          'lastExportedLocalId': lastMessage?.localId ?? 0,
          'lastExportedTimestamp': lastMessage?.createTime ?? 0,
          'isIncremental': onlyNewMessages && existingState != null,
          'newMessagesCount': newMessageItems.length,
        },
      };
      ...
      // 创建新的导出状态
      final newState = ExportState(
        sessionWxid: session.username,
        displayName: session.displayName ?? session.username,
        filePath: filePath,
        format: 'json',
        lastExportedLocalId: lastMessage?.localId ?? 0,
        lastExportedTimestamp: lastMessage?.createTime ?? 0,
        totalExportedCount: allMessages.length,
        firstExportTime:
            existingState?.firstExportTime ?? DateTime.now(),
        lastExportTime: DateTime.now(),
        history: [
          ...(existingState?.history ?? []),
          ExportHistory(
            exportTime: DateTime.now(),
            addedCount: newMessageItems.length,
            totalCount: allMessages.length,
            lastLocalId: lastMessage?.localId ?? 0,
          ),
        ],
      );

      // 保存导出状态
      await ExportStateService.saveExportState(newState);

      await logger.info(
        'ChatExportService',
        '增量导出 JSON 成功: wxid=${session.username}, '
        'new=${newMessageItems.length}, total=${allMessages.length}',
      );
  ...
  Map<String, dynamic> _buildPublicMessagePayload(
    Message msg,
    String senderName,
  ) {
    return {
      'formattedTime': msg.formattedCreateTime,
      'type': _resolveMessageType(msg),
      'content': msg.displayContent,
      'senderDisplayName': senderName,
    };
  }

  Map<String, dynamic> _normalizeLegacyMessagePayload(
    Map<String, dynamic> raw,
  ) {
    return {
      'formattedTime': _safeString(
        raw['formattedTime'] ?? raw['createTime'],
      ),
      'type': _safeString(
        raw['type'] ?? raw['localType'],
      ),
      'content': _safeString(raw['content']),
      'senderDisplayName': _safeString(
        raw['senderDisplayName'] ?? raw['senderUsername'],
      ),
    };
  }

  String _resolveMessageType(Message msg) {
    final typeDesc = msg.typeDescription;
    if (typeDesc != null && typeDesc.trim().isNotEmpty) {
      return typeDesc;
    }
    final localType = msg.localType;
    if (localType != null) {
