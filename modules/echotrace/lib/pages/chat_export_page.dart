  ...
  Future<void> _startExport() async {
    ...
    for (int i = 0; i < widget.sessions.length; i++) {
      ...
      try {
        ...
        // JSON 格式不带时间戳（支持增量导出）
        // HTML 和 Excel 带时间戳（不支持增量导出）
        final filePath = widget.format == 'json'
            ? '${widget.exportFolder}${Platform.pathSeparator}$sanitizedName.$extension'
            : '${widget.exportFolder}${Platform.pathSeparator}${sanitizedName}_${DateTime.now().millisecondsSinceEpoch}.$extension';
